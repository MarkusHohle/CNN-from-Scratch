            ...
            for c in range(numChans):# loop over channels
                for y in range(yd):# loop over vert axis of output
                    ...
                    #determining the edges of the window
                        ...
                        #countdown, see slides in ppt
                        mask_orig_copy[sx,sy,c,i] -= mask[sx,sy,c,i]
                        mask_copy[sx,sy,c,i] = np.round(mask_orig_copy[sx,sy,c,i]/(mask_orig_copy[sx,sy,c,i] + 1e-7))
...
class Sigmoid:
    ...
    def forward(self,M):
        ...
        self.output = sigm
        self.inputs = sigm #needed for backprop later
